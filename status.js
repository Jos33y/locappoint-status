...
(function () {
    ...
    // Reveal triggers on viewport entry. Children cascade with stagger.

    const CHILD_SELECTORS = '.card, .check, .step, .beyond__item, .next-card, .payment, .need, .log__entry, .quicklink, .phase, .city, .email-row:not(.email-row--head)'
    const STAGGER_MS = 55
        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach((entry) => {
                ...
                items.forEach((item, i) => {
                    ...
                    // rAF so the class lands on the next paint, not the current one.
